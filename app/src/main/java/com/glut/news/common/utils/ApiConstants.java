package com.glut.news.common.utils;

/**
 * Created by yy on 2018/1/29.
 */

public class ApiConstants {
    public final static String welcomeImageAPi="https://www.dujin.org/sys/bing/1920.php";
    public final static String NewsListApi="";
    public final static String VideoListApi="http://ips.ifeng.com/video19.ifeng.com/video09/2017/07/25/4710314-102-008-0939.mp4";
//凤凰新闻API
    public static final String sIFengApi = "http://api.iclient.ifeng.com/";

    public static final String sJanDanApi = "http://i.jandan.net/";

    public static final String sNewsListApi="http://api.3g.ifeng.com/api_wemedia_list?cid=722080";
    public static final String sGetNewsImagesApi = "NewRelativeVideoList";

    public static final String sGetNewsArticleCmppApi = "http://api.3g.ifeng.com/";

    public static final String sGetNewsArticleDocCmppApi = "ipadtestdoc";

    public static final String TouTiaoApi="http://ic.snssdk.com/2/article/v25/stream/?count=20&min_behot_time=1504621638&bd_latitude=4.9E-324&bd_longitude=4.9E-324&bd_loc_time=1504622133&loc_mode=5&loc_time=1504564532&latitude=35.00125&longitude=113.56358166666665&city=%E7%84%A6%E4%BD%9C&lac=34197&cid=23201&iid=14534335953&device_id=38818211465&ac=wifi&channel=baidu&aid=13&app_name=news_article&version_code=460&device_platform=android&device_type=SM-E7000&os_api=19&os_version=4.4.2&uuid=357698010742401&openudid=74f06d2f9d8c9664";
}
