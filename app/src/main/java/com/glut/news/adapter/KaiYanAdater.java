...
public class KaiYanAdater extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener{
    ...
    private Context c;
    private List<KaiYan.KaiYanList> kaiyanList;
    ...
    public KaiYanAdater(FragmentActivity activity, List<KaiYan.KaiYanList> kaiYanList) {
    ...
    public void changeData(List<KaiYan.KaiYanList> stories) {
        this.kaiyanList=stories;
        notifyDataSetChanged();
    }

    public void addData(List<KaiYan.KaiYanList> stories) {
...
if (kaiyanList.get(position).getType().equals("video")){
    ...
    if (kaiyanList.get(position).getData().getAuthor()!=null && kaiyanList.get(position).getData().getAuthor().getIcon()!=null){
        ...
        Glide.with(c).load(kaiyanList.get(position).getData().getAuthor().getIcon()).transform(new CircleImage(c)).into(  ((ItemViewHolder) holder).author_logo);
