...
public class OneFragment extends android.support.v4.app.Fragment implements PullToRefreshView.OnRefreshListener {
    ...
    private List<OneData.OneList> oneList = new ArrayList<>();
    ...
    public void loadDta() {
        ...
        RetrofitManager.builder(RetrofitService.ONE_MOMENT_BASE,"OneService").getOneDateList()
                ...
                .map(new Func1<OneDateList, OneDateList>() {
                    @Override
                    public OneDateList call(OneDateList douBanList) {
                        return douBanList;
                    }
                })
                .subscribe(new Action1<OneDateList>() {
                    @Override
                    public void call(OneDateList oneDateList) {
                        mPbLoading.setVisibility(View.GONE);
                        if (oneDateList==null){
                            mTvLoadEmpty.setVisibility(View.VISIBLE);
                        }else{

                            currentDate=Integer.parseInt(oneDateList.getData().get(0))-1;
                            loadData2(oneDateList.getData().get(0));
    ...
    private void loadData2(String s) {
        ...
        RetrofitManager.builder(RetrofitService.ONE_MOMENT_BASE,"OneService").getOneLasteNews(s)
                ...
                .map(new Func1<One, One>() {
                    @Override
                    public One call(One one) {
                        return one;
                    }
                })
                .subscribe(new Action1<One>() {
                    @Override
                    public void call(One one) {
                        ...
                        if (one==null){
                            ...
                            oneAdater.changeData(one.getData().getContent_list());
                            mTvLoadEmpty.setVisibility(View.GONE);
                        }
                        mLoadLatestSnackbar.dismiss();
                        refreshLayout.setRefreshing(false);
                ...
                .subscribe(new Action1<One>() {
                    ...
                    public void call(One one) {
                        ...
                        if (one==null){
                            ...
                            oneAdater.addData(one.getData().getContent_list());
                            mTvLoadEmpty.setVisibility(View.GONE);
                        }
